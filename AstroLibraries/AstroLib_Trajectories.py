...
class shapingMethod:
    ...
    def calculateVel(self, N, gamma1,  r1, r2, psi,):
        k2 = self.k2
        steps = 300

        theta_b = psi + 2 * np.pi * N

        TOF, param = self.S(k2,r1, r2, psi, N, gamma1, steps)
        ...
        theta_dot_2 = np.sqrt( self.mu/r2**3 / \
                    (np.tan(gamma2)**2 + k1*k2**2*np.sin(k2*theta_b+phi) + 1) )
    ...
    def plot_sphere(self, r1, r2, psi, gamma1, N):
        ...
        theta = np.linspace(0, 2*np.pi*(N+1), 2000)
        r = np.zeros((len(theta)))
        x = np.zeros((len(theta)))
        y = np.zeros((len(theta)))

        for i in range(len(theta)):
            r[i] = k0 * np.exp( k1*np.sin(k2*(theta[i]) +phi ) )
